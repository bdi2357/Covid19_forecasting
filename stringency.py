...
def integrate_frames(strg,ind_strg,dc,ind_dc):
    ...
    ind_strg.to_csv("../index_strng_%s_%s.csv"%(os.path.basename(__file__),inspect.stack()[0][3]),index=False)
    ind_dc.to_csv("../index_covidf_%s_%s.csv"%(os.path.basename(__file__),inspect.stack()[0][3]),index=False)
    strg.to_csv("../strng_%s_%s.csv"%(os.path.basename(__file__),inspect.stack()[0][3]),index_label="index")
    dc.to_csv("../covidf_%s_%s.csv"%(os.path.basename(__file__),inspect.stack()[0][3]),index_label="index")

    LL = (set(ind_dc["index"]).intersection(set(ind_strg["index"])))
    MM = sorted(list(LL))

    ind_strng_d = ind_strg.set_index("index")["val"].to_dict()
    ind_dc_d = ind_dc.set_index("index")["val"].to_dict()
    ...
    DF_ind.to_csv("../index_try11.csv",index=False)#%(os.path.basename(__file__),inspect.stack()[0][3]))
...
if __name__ == "__main__":
    ...
    print(ind_strg.tail())
    print(ind_dc.tail())
    strg = pd.read_csv("../strng_stringency.py_integrate_frames.csv",index_col="index")
    dc  = pd.read_csv("../covidf_stringency.py_integrate_frames.csv",index_col="index")
    print("download time is %0.2f" %(time.time()-start))
    integ_st = time.time()
    c,d = integrate_frames(strg,ind_strg,dc,ind_dc)
    print(c.shape)
    print(d.head(),d.tail())
    df3 = pd.read_csv("../index_try11.csv")
    print("$"*55)
    print(df3.head(),df3.tail())

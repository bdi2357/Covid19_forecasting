...
d = [[1,2,34,77,6],[1,2,99,46],[1,2,77,44]]
...
D1 = pd.DataFrame(data=[[33,22],[12,33],[55,44]], columns =['a','b'])
D2 = pd.DataFrame(data=[[77,22],[92,33],[40,44]] ,columns =['a','c'])
D3 =  pd.DataFrame(data=[[77,22],[92,33],[40,44]], columns =['a','c'])
AA= pd.concat([D1,D2,D3],axis=1)
AA
...
len(set(strg_f.index.values)- set(dc_f.index.values)),len(set(dc_f.index.values)- set(strg_f.index.values))
